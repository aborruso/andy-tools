...
- `tools/wopen`: gli URL con `&` non si aprivano, e quelli con `;`, `,`, spazio, `{` o `}` si aprivano troncati dichiarando successo. `open_via_powershell` passava il target come argomento in coda a `powershell.exe -Command`, che non lo tratta come parametro: concatena gli argomenti residui al testo dello script e li interpreta. Il target finiva nel sorgente PowerShell, non nei dati. Un URL semplice funzionava per caso, come token nudo legato a `param($p)`, ed è per questo che il difetto è rimasto nascosto. Misurato: errore rumoroso con `&`, `'`, `(`, `)`, `|`; troncamento silenzioso con exit 0 su `;`, `,`, spazio, `{`, `}`. Stesso difetto sui percorsi Windows, che passano dalla stessa funzione (`C:\tmp\x&y.txt`); il ramo dei file Linux usa `cmd.exe /c start "" "$target"` e le virgolette lo proteggono, verificato. Ora il target viaggia in una variabile d'ambiente esportata con `WSLENV` (nome nudo, nessun flag, appeso a un `WSLENV` preesistente) e lo script PowerShell è a testo costante: niente escape scritto a mano, nessuna nuova dipendenza. Round-trip byte-esatto verificato su 14 casi, `&` `;` `,` spazio `'` `"` `$` backtick `|` `(` `)` `{` `}` `%20` `%26` e un percorso Windows. Il `try/catch` con `exit 1` mantiene l'exit code non-zero sul fallimento, che prima arrivava dall'errore di parsing e sarebbe altrimenti sparito: `wopen 'C:\does\not\exist\x&y.txt'` esce 1 con il messaggio di Windows. Prova end-to-end nel browser con `https://httpbin.org/get?a=1&b=2&c=tre%20parole`: la pagina riceve tutti e tre i parametri. Resta latente e non toccata l'espansione di `%VAR%` da parte di `cmd.exe` in un nome di file.

## 2026-08-29

- `openspec`: change `2026-08-29-wopen-fix-clipboard-encoding` aperta e archiviata, la capability `copy-to-clipboard` ora ha due requirement sulla codifica (UTF-16LE senza BOM, fallback non-UTF-8). Per archiviarla ho dovuto sistemare `openspec/specs/copy-to-clipboard/spec.md`, che non aveva le sezioni `## Purpose` e `## Requirements`: senza, i requirement esistenti erano invisibili a `validate`, `list` e `archive`.

- `tools/wopen`: `wopen -c` corrompeva ogni carattere non ASCII. `clip.exe` legge i byte da stdin nella codepage ANSI, quindi `è` finiva in clipboard come `├¿`; il file su disco restava integro e il danno si vedeva solo dopo aver incollato. Ora il contenuto passa per `iconv -f UTF-8 -t UTF-16LE` in un file temporaneo, e solo se la conversione riesce va a `clip.exe`; se il file non è UTF-8 valido si torna ai byte grezzi con un avviso su stderr (exit 0, comportamento invariato rispetto a prima). Niente BOM: verificato che `clip.exe`, se lo riceve, lo copia come carattere U+FEFF in testa al testo, mentre senza BOM riconosce lo stesso l'UTF-16LE. Un eventuale BOM UTF-8 nel sorgente viene tolto prima della conversione. Verificato dal vivo leggendo i code point con `Get-Clipboard`: `perché così è più` → 233/236/232/249 corretti, file con BOM → nessun FEFF residuo, giapponese ed emoji (coppia surrogata) integri, stringhe ASCII brevi (`ls`, `a`) ancora corrette, file senza newline finale copiato senza newline aggiunta (`ciao è` → 99,105,97,111,32,232). Il `\n` finale diventa CRLF in clipboard come prima del fix.

## 2026-08-24

- `tools/istatdata-ai` 1.0.0: nuovo harness CLI-Anything sopra la ricerca IA di IstatData (`istat-ask`, alias `cli-anything-istatdata-ai`). Endpoint ricostruito dal bundle del Data Browser e verificato dal vivo: `POST databrowserhub/api/core/nodes/1/AI/ExecuteSearch`, pubblico, con tre trappole — `aiRateLimiting` è obbligatorio (senza, HTTP 200 con `INTERNAL_ERROR_SERVER` e messaggio vuoto), la lingua passa da un header proprietario `UserLang` e non da `Accept-Language`, e con `UserLang: en` il backend restituisce `title` e `ai_title` vuoti. Da qui il join con `nodes/1/catalog` (3282 dataset, cache 24 h in `~/.cache/istatdata-ai/`, una per nodo e lingua): su 100 id restituiti da 5 domande diverse, 0 mancanti. Il deep link alla tavola richiede il percorso di categoria completo, id del gruppo compreso — senza, l'app risponde "pagina non disponibile"; verificato aprendo sia un percorso a due sia uno a tre segmenti. In output anche gli URL SDMX (struttura e dati CSV). `session_id` rimandato indietro porta il contesto della conversazione (verificato: "quanti sono gli occupati in Sicilia" → "e per le donne?" dà `conversation` di 4 turni). Non implementati stato di sessione, undo/redo e `--dry-run` prescritti dall'harness: il target è in sola lettura. Se il catalogo non risponde, `ask` risponde lo stesso: avviso su stderr e risultati senza link. Test: 22 offline su fixture, 31 con la suite live (`ISTATDATA_AI_LIVE=1`).
